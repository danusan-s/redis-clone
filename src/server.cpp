...
// struct to describe a connection
struct Conn {
  int fd = -1;
...
// application callback when the listening socket is ready to accept
static Conn *handle_accept(int fd) {
  ...
  // create a new connection and read the first request
  Conn *conn = new Conn();
  conn->fd = connfd;
  conn->want_read = true;
  return conn;
...
static bool try_one_request(Conn *conn) {
  // Check if len part is fully read into buffer
  ...
  // Check if the message is fully read into buffer
  ...
  // Generate the response (simply echo for now)
...
static void handle_write(Conn *conn) {
  ...
  // Not empty the buffer as a single connection could have made multiple
  // requests and we don't want to lose their data
...
int main() {
  ...
  // we use a vector since fds are small integers and packed
  ...
  while (true) {
    ...
    // block until an event happens on any of the sockets
